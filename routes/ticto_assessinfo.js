...
router.post('/', function(req,res){
    ...
    console.log("TICTO assessinfo API 연동 START! ");
    console.log(req);
    console.log(typeof(req));
    console.log("TICTO assessinfo API 연동 END!");
    
    res.json({"resultCode":0, "resultString": "전송 되었습니다."})
})
...
// 연속 점수 데이터
function cons_score(list){
    var max_cons_count = find_array_max(list);
    var cons_score = 0;
    var basic_score = 0.2;
    var cons_weight = 0.1;

    for(var i=1; i <= max_cons_count; i++){
        cons_score = cons_score + basic_score + (basic_score + cons_weight);
    }
    return cons_score;
}
...
// 주의력
function GNG_A(featureList){
    
    var CorrTar = parseInt(featureList.gngNumOfCorrTar);
    var CorrNonTar = parseInt(featureList.gngNumOfCorrNonTar);

    var TargerAntiRes = 0
    // AnticipantResponse
    for(var i=0; i < featureList.gngRndSuccessList.length; i++){
        var ResponseTime = parseFloat(featureList.gngResponseTimeListCalc[i]);
    ...
    var basic_score = (CorrTar + CorrNonTar - TargerAntiRes) / 20;

    return basic_score * (0.25)
}
...
// 주의력
function SYMBOLSEARCH_01(featureList){
...
//     "pmVld":true,
//     "pmPerformedRound":6,
//     "pmNumOfSuccessRound":3,
//     "pmTryTotalNumList":[5,8,11,11,14,14],
//     "pmTrySuccessNumList":[5,8,6,11,6,6],
//     "pmTryErrStateList":[0,0,2,0,2,4],             
//     "pmCnsctvSuccessList":[1],
//     "pmResponseTimeListCalc":[9.82,13.55,18.67,18.95,11.92,23.02],
//     "pmPerformanceTime":98.1
// }

// 주의력
function PM_03(featureList){
